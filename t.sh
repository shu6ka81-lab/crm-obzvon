...
#
# Ответ 000 здесь не приговор: сам сервер до Cloudflare достаёт через раз,
# тогда как снаружи ссылка открывается. Поэтому это подсказка, а не проверка.
say "Проверка снаружи"
CODE="$(curl -s -o /dev/null -w '%{http_code}' --max-time 25 "$URL/login" 2>/dev/null || true)"
case "${CODE:-000}" in
  200|30[0-9]) echo "ответ по ссылке: HTTP $CODE — открывается" ;;
  000|'')      echo "с самого сервера ссылка не открылась — проверьте её из браузера" ;;
